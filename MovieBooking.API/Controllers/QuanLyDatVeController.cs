...
namespace MovieBooking.API.Controllers
{
    ...
    public class QuanLyDatVeController : ControllerBase
    {
            ...
            [FromHeader(Name = "TokenCybersoft")] string tokenCybersoft)
        {
            ...
            if (DanhSachVe == null)
                return BadRequest(ApiResponse<string>.Error("DanhSachVe không h?p l?"));
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(taiKhoan))
                    return Unauthorized(ApiResponse<string>.Error("Không xác th?c ðý?c tài kho?n", 401));

                await _datVeService.DatVeAsync(DanhSachVe, taiKhoan);
                return Ok(ApiResponse<string>.Success("Ð?t vé thành công", "Success"));
            }
            catch (Exception ex)
            {
            ...
            [FromQuery(Name = "MaLichChieu")] int MaLichChieu = 0)
        {
            ...
            if (data == null)
                return NotFound(ApiResponse<string>.Error("Không t?m th?y l?ch chi?u", 404));
            ...
            [FromHeader(Name = "TokenCybersoft")] string tokenCybersoft)
        {
            ...
            if (lich == null)
                return BadRequest(ApiResponse<string>.Error("D? li?u l?ch chi?u không h?p l?"));
